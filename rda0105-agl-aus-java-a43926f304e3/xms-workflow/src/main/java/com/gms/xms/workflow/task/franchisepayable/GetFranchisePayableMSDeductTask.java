...
public class GetFranchisePayableMSDeductTask implements Task {
    private static final Log log = LogFactory.getLog(GetFranchisePayableMSDeductTask.class);
    ...
    public boolean execute(ContextBase context) throws Exception {
        ...
        try {
            ...
            List<FranchisePayableMSDeductVo> result = fpbMSDao.getCarrierCostDeduction(filter);

            // Puts result into the context
            context.put(Attributes.FRANCHISE_PAYABLE_CARRIER_COST_DEDUCTION_RESULT, GsonUtils.toGson(result));

            // Do DAO service to get Carrier Cost Deduction records count.
            Integer recordCount = fpbMSDao.getCarrierCostDeductionRecordCount(filter);

            // Puts it into the context.
            context.put(Attributes.FRANCHISE_PAYABLE_CARRIER_COST_DEDUCTION_RECORD_COUNT_RESULT, String.valueOf(recordCount));
            ...
            if ((startRecord != null && recordSize != null && (recordCount - startRecord) <= recordSize) || (startRecord == null || recordSize == null)) {
                // Get summary record.
                FranchisePayableMSDeductVo totalResult = fpbMSDao.getCarrierCostDeductionTotal(filter);

                // Puts it into the context.
                context.put(Attributes.FRANCHISE_PAYABLE_CARRIER_COST_DEDUCTION_TOTAL_RESULT, GsonUtils.toGson(totalResult));
            }
        } catch (Exception e) {
