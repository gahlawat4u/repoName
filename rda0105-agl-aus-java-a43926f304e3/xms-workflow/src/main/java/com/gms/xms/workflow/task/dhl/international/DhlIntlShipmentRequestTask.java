...
public class DhlIntlShipmentRequestTask implements Task {
    private static final Log log = LogFactory.getLog(DhlIntlShipmentRequestTask.class);
        ...
        try {
            ...
            if (shipmentRequestResult.getErrorResponse() == null && shipmentRequestResult.getShipmentValidateErrorResponse() == null) {
                ...
                for (ShipValResponsePiece piece : pieces) {
                    ...
                    savePiece.setDimensionL(piece.getDepth());
                    savePiece.setDimensionH(piece.getHeight());
                    savePiece.setDimensionW(piece.getWidth());
                    savePiece.setWeight(piece.getWeight().setScale(2, RoundingMode.HALF_UP).doubleValue());
