...
public class SendEmailCancelStartrackScheduleCollectionTask implements Task2 {
    private static final Log log = LogFactory.getLog(SendEmailCancelStartrackScheduleCollectionTask.class);

    @Override
    public boolean execute(ContextBase2 context) throws Exception {
        context.put(Attributes.ERROR_CODE, ErrorCode.SUCCESS);
        Map<String, Object> dataSend  = context.get(Attributes.CANCEL_COLLECTION_DATA_MAPPING);
        String outputFilePath = AppConstants.APP_SETTINGS.getAppTmpPath() + "/startrack_cancel_schedule_email_" + System.currentTimeMillis()+ ".txt";
        AppUtils.template2File(outputFilePath, false, "templates/email/startrack/startrack_cancel_schedule_collection.ftl", dataSend);
        String content = AppUtils.readUTF8File2String(outputFilePath);
        String subject = "Startrack Void for " + context.get(Attributes.AIRBILL_NUMBER);
