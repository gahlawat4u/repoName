...
public class GenFileViewManifestTask implements Task2 {
    ...
    public boolean execute(ContextBase2 context) throws Exception {
        try {
            ...
            String fileName = fileViewName + shipmentId + miniTime;

            LinkedList<String> htmlFilePaths = new LinkedList<>();
            String htmlFilePath1 = AppConstants.APP_SETTINGS.getAppTmpPath() + "/" + fileName + "1.html";


            htmlFilePaths.addLast(htmlFilePath1);
            String htmlFilePath2 = AppConstants.APP_SETTINGS.getAppTmpPath() + "/" + fileName + "2.html";
            htmlFilePaths.addLast(htmlFilePath2);
            Map<String, Object> data = context.get(Attributes.VIEW_AIRBILL_DATA);
            AppUtils.template2File(htmlFilePath1, false, "templates/pdf/history/tnt_view_manifest/tnt_view_manifest_page_1.ftl", data);
            AppUtils.template2File(htmlFilePath2, false, "templates/pdf/history/tnt_view_manifest/tnt_view_manifest_page_2.ftl", data);
            String pdfFilePath = AppConstants.APP_SETTINGS.getAppTmpPath() + "/" + fileViewName + shipmentId + miniTime + ".pdf";

            AppUtils.createPDFFromHTMLWithFont(htmlFilePaths, pdfFilePath, "arial", true);
