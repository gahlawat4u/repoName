package com.gms.xms.workflow.task.directcouriers;
 ...
 * @author tkvcl
 */
public class GetDirectCouriersZoneTask implements Task {
    private static final Log log = LogFactory.getLog(GetDirectCouriersZoneTask.class);
    ...
    public boolean execute(ContextBase context) throws Exception {
        ...
        try {
            ...
            if (shipmentTypeVo != null) {
                if (shipmentTypeId == 1 && (!shipmentTypeVo.getShipmentTypeName().trim().equals("DHL ETS") || shipmentTypeVo.getShipmentTypeName().trim().equals("ETS"))) {
                    context.put(Attributes.ERROR_CODE, ErrorCode.ERROR);
                    context.put(Attributes.ERROR_MESSAGE, message);
                    return false;
                }

                if (!Arrays.asList(serviceAvaible).contains(shipmentInfoVo.getShipmentId().toString())) {
                    context.put(Attributes.ERROR_CODE, ErrorCode.ERROR);
                    context.put(Attributes.ERROR_MESSAGE, message);
                    return false;
                }
