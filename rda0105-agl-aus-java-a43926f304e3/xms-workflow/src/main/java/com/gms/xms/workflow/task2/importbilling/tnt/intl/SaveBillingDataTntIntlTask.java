...
/**
 * Posted from May 25, 2016 11:35:35 AM
 * <p>
 * Author huynd
 */
public class SaveBillingDataTntIntlTask implements Task2 {

    private static final Log log = LogFactory.getLog(SaveBillingDataTntIntlTask.class);
    ...
    public boolean execute(ContextBase2 context) throws Exception {
        try {
            ...
            Long carrier = 54L;
            Long senderCountryId, receiverCountryId;
            ...
            String description = "", displayDescript = "";
            String isContent = "";
            ...
            for (BillingRowDataModel rowData : billingRowData) {
                ...
                senderCountryId = (countryDao.getCountryIdByCountryCode(cellData.get("sender_countrycode")) == null) ? 0 : countryDao.getCountryIdByCountryCode(cellData.get("sender_countrycode"));
                senderAddress.setCountry(senderCountryId);
                senderAddress.setCompanyName(cellData.get("shipper_name"));
                ...
                receiverCountryId = (countryDao.getCountryIdByCountryCode(cellData.get("receiver_countrycode")) == null) ? 0 : countryDao.getCountryIdByCountryCode(cellData.get("receiver_countrycode"));
                ...
                if (description.toLowerCase().contains("inbound")) {
                    description = description.replace("- Package (Inbound)", "");
                    displayDescript = description.trim() + " (Inbound)";
                    billingBound = true;
                    boundStatus = 1;
                } else {
                    description = description.replace("- Package", "");
                    displayDescript = description.trim();
                }
                shipmentTypeFilter = new ShipmentTypeFilter();
                shipmentTypeFilter.setShipmentTypeName(description);
                shipmentTypeFilter.setServiceId(54);
                shipmentTypeVo = shipmentTypeDao.selectByShipmentTypeNameAndCarrier(shipmentTypeFilter);
                ...
                if (checkShipment != null) {
                    ...
                    if (!StringUtils.isBlank(checkAirbillNumber)) {
                        ...
                        if (billingBound) {
                            displayDescript = checkShipment.getOrgShipmentTypeName() + " (Inbound)";
                        }
                ...
                // Fuel surcharge
                if (!StringUtils.isBlank(cellData.get("fuel_surcharge")) && !StringUtils.isBlank(cellData.get("fuel_surcharge_cost")) && Double.valueOf(cellData.get("fuel_surcharge_cost")) != 0) {
                    ...
                    tax = (defaultTaxPercent == null) ? 0D : defaultTaxPercent;
                    carrierCost = Double.valueOf(cellData.get("fuel_surcharge_cost"));
                    carrierTaxPercent = (defaultTaxPercent == null) ? 0D : defaultTaxPercent;
                    taxAmount = carrierCost * carrierTaxPercent / 100;
                    franchiseCost = carrierCost;
                    franchiseTaxAmount = franchiseCost * carrierTaxPercent / 100;
