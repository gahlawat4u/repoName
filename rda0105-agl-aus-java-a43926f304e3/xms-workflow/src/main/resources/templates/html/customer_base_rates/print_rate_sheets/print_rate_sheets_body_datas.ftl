<div style="text-align: center;"><span
        style="font-size: 13px;font-weight: bold; color: #4861AD">${(rs["rateSheet"].title?html)!" "}</span></div>
<div><span
        style="font-weight: bold; color: #4861AD">${(lang.translate("Value in"))!"Value in"} ${(currencyCode)!"AUD"}</span>
</div>
<#if rs["rateSheet"]??>
<table class="border">
    <thead>
...
<#if rs["perWeightRateSheet"]??>
<div><span
        style="font-weight: bold; color: #4861AD">${(lang.translate("Non-Document above {weight} kg (Multiply shipment weight by zone rate"))?replace("{weight}",rs["rateSheet"].maxWeight)}</span>
</div>
<table class="border">
    <thead>
    <tr>
