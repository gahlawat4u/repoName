$(function () {
    ...
    /*
     * if ($.cookie('style')) { list_style.find('option').each(function(){
     * if($(this).attr('value') == $.cookie('style')) { $(this).attr('selected',
     * 'selected'); } }); list_color.removeClass("active");
     * list_color.each(function(){ if($(this).attr('data-color') ==
     * $.cookie('color')){ $(this).addClass('active'); } });
     * setTheme($.cookie('style'), $.cookie('color')); }; // SELECT EVENT
     * list_style.on('change', function() { list_color.each(function() {
     * if($(this).hasClass('active')){ color_active =
     * $(this).attr('data-color'); } }); setTheme($(this).val(), color_active);
     * }); // LI CLICK EVENT list_color.on('click', function() {
     * list_color.removeClass('active'); $(this).addClass('active');
    ...
    $('.chat-textarea input').on("keypress", function (e) {
        ...
        if (e.which == 13) {
            ...
            if ($content !== "") {
                ...
                setTimeout(function () {
                    $me.append($res);
                    $me.scrollTop(height + 100); // add more 500px for
                    // #chat-box position
                }, 1000);
            }
        }
    ...
    (function ($) {
        $(document).ready(function () {
            ...
            $('#cssmenu>ul').prepend('<li id="menu-button"><a style="opacity:0 !important; " >.</a></li>');
    ...
    $('#run-ship').click(function () {
        BootstrapDialog.show({
            message: $('<div></div>').load('modal_dialog/modal_ship.html')
        });
    });
    $('#run_shipment_detail').click(function () {
        BootstrapDialog.show({
            message: $('<div></div>').load('modal_dialog/modal_shipment_detail.html')
        });
    });
    $('#run_send_airbill').click(function () {
        BootstrapDialog.show({
            message: $('<div></div>').load('modal_dialog/modal_sent_airbill.html')
        });
    });
    $('#run_quote_detail').click(function () {
        BootstrapDialog.show({
            message: $('<div></div>').load('modal_dialog/modal_quote_detail.html')
        });
    });
    $('#run_lookup').click(function () {
        BootstrapDialog.show({
            message: $('<div></div>').load('modal_dialog/modal_lookup.html')
        });
    });
    $('#run_quote').click(function () {
        BootstrapDialog.show({
            message: $('<div></div>').load('modal_dialog/modal_quote.html')
        });
    });
    $('#run_quote1').click(function () {
        BootstrapDialog.show({
            message: $('<div></div>').load('modal_dialog/modal_quote1.html')
        });
    });
    $('#run_map').click(function () {
        BootstrapDialog.show({
            message: $('<div></div>').load('modal_dialog/modal_map.html')
        });
    });
    ...
    $("#lookup-link").click(function (event) {
        $("#lookup").dialog("open");
    ...
    // Link to open the dialog
    $("#ship-link").click(function (event) {
        $("#ship").dialog("open");
        event.preventDefault();
    });
    $("#note").dialog({
    ...
    // Link to open the dialog
    $("#addnote-link").click(function (event) {
        $("#addnote").dialog("open");
        event.preventDefault();
    });
    ...
    // Link to open the dialog
    $("#login-ass-link").click(function (event) {
        $("#login-ass").dialog("open");
        event.preventDefault();
    });
    // END-dialog UI
});
