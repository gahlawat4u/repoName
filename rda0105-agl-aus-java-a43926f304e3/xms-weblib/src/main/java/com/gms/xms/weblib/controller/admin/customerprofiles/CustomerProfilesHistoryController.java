...
/**
 * Posted from CustomerProfilesHistoryController
 * <p/>
 * Author TANDT 26-10-2015
 */
public class CustomerProfilesHistoryController extends CustomerProfilesAccountSetupController {
    ...
    private void prepareHistoryLog() throws Exception {
        ...
        long countEventlog = service.selectByProfileIdCount(filter);
        Paging<EventLogModel> paging = new Paging<EventLogModel>(filter.getPage(), nLinks, filter.getPageSize(), countEventlog);
        List<EventLogVo> eventLogVos = service.selectByProfileId(filter);
