...
/**
 * Posted from Apr 5, 2016 10:34:37 AM
 * <p>
 * Author dattrinh
 */
public class ManageCustBaseRateController extends JsonBaseController {
    ...
    public String doSaveBaseRate() {
        try {
            if (this.getSaveCustBaseRate() == null) {
                throw new CustomException("No customer base rates information for saving.");
            }
            // Convert to vo.
            SaveCustomerBaseRateVo saveCustomerBaseRateVo = ModelUtils.createVoFromModel(this.getSaveCustBaseRate(), SaveCustomerBaseRateVo.class);
            IBaseRateService baseRateService = new BaseRateServiceImp();
            baseRateService.saveCustBaseRates(this.getAddInfoMap(), saveCustomerBaseRateVo);
        } catch (Exception e) {
            handleError(e, AppConstants.SYSTEM_ERROR_MESSAGE);
        }
        return "success";
    ...
    private void loadCustomer() throws Exception {
        ...
        CustomerVo customerVo = customerService.selectByCode(this.getCustomerCode());
    ...
    protected void loadDHL() throws Exception {
        ...
        if (showServiceTypes.size() > 0) {
            for (ServiceTypeVo serviceTypeVo : showServiceTypes) {
                ...
                loadCustBaseRates(this.getCustomerCode(), serviceTypeVo);
            }
        }
        // Set to model.
    ...
    protected void loadOthers() throws Exception {
        ...
        List<OtherCustBaseRateVo> baseRateVos = dao.getOtherBaseRateByCustomerCode(this.getCustomerCode());
    ...
    private void loadUPS() throws Exception {
        ...
        for(ServiceTypeVo serviceTypeVo : serviceTypesFull){
            if(serviceTypeVo.getContent()==1 || serviceTypeVo.getContent()==0 )
            {
    ...
    private List<RateType> prepareRateTypes(Double weight, Integer serviceId) {
        ...
        switch (serviceId)
        {
            ...
            case 400:
                ...
                break;
            case 72:
                rateTypes.add(new RateType(0, "Star Track"));
                break;
            case 52:
                rateTypes.add(new RateType(0, "Toll Priority"));
                break;
            case 59:
                rateTypes.add(new RateType(0, "Toll Ipec"));
                break;
                
        }

       /* rateTypes.add(new RateType(1, "% Markup"));*/
        rateTypes.add(new RateType(1, "% Margin"));
        /*if(weight!=0)
        {
