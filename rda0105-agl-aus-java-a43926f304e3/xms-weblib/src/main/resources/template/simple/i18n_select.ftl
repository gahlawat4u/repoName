...
<#if parameters.headerKey?? && parameters.headerValue??>
    <option value="${parameters.headerKey?html}"
        <#if tag.contains(parameters.nameValue, parameters.headerKey) == true>
            ...
            >${parameters.taglibHelper.getLocalizationText(parameters.headerValue)?html}</option>
</#if>
...
<@s.iterator value="parameters.list">
    ...
    <option value="${itemKeyStr?html}"<#rt/>
        ...
        <#if itemTitle?has_content>
            ...
            >
        <#if parameters.i18nitem?? && parameters.i18nitem=="no">
    	${itemValue?html}
    <#else>
        ${parameters.taglibHelper.getLocalizationText(itemValue)?html}
        </#if>
    </option><#lt/>
</@s.iterator>
