        ...
        <tr>
            <th class="s42" class="text-center" valign="middle"><span style="margin-left: 10px;"><xms:localization
                    text="Show"/></span> <s:i18n_select name="filter.pageSize" list="pageSizeList" cssClass="s52"
                                                        ...
                                                        cssStyle="height: 22px; padding-top: 1px;" i18nitem="no"/>
    ...
    <table class="table table-bordered mg0" id="datatable1">
        ...
        <s:else>
            <s:iterator value="adjustmentList.records" status="row">
                <tr>
                    <td class="text-center"><s:if test="userLevel>3 && status==7">
                    </s:if> <s:else>
                        <s:if test="status!=6">
                            <a href="javascript:editAdj(<s:property value='adjustmentRequestId'/>);"
                               title='<xms:localization text="Edit"/>'><i class="fa fa-edit"></i></a>
                            <a href="javascript:showNoteAdjDlg(<s:property value='adjustmentRequestId'/>,1);"
                               title='<xms:localization text="Delete"/>'><i class="fa fa-trash-o"></i></a>
                        </s:if>
                    </s:else> <s:if test="status==3">
                    ...
                    <td align="center" <s:if test="status==1">
                        ondblclick="updateSubStatus(<s:property value="adjustmentRequestId"/>)"
                        style="cursor: pointer;"
                        title='<xms:localization text="Double click to change status"/>'
                    </s:if>><s:i18n_property
                            value="statusName"/></td>
                    <td><s:property value="reasonForDeleting"/></td>
                    <td><s:property value="note"/></td>
                    <td <s:if test="status!=6">
                        onclick="showNoteAdjDlg(<s:property value='adjustmentRequestId'/>,2)"
                    ...
                    <s:if test="userLevel<3">
                        <td <s:if test="status!=6">
                            onclick="showNoteAdjDlg(<s:property value='adjustmentRequestId'/>,3)"
        ...
        <tr>
            <td colspan="<s:if test="userLevel<3">14</s:if><s:else>12</s:else>"><span class="b4"><b><xms:localization
                    text="Total Adjustments:"/></b> <s:property value="total.recordCount"/></span> <span class="b4"><b>|
                <xms:localization text="Franchise Request Total:"/>
