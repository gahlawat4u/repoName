...
<table class="table table-bordered mg0" id="weekly_report_table">
    ...
    <s:else>
        <s:iterator value="weeklyReport" var="weekly">
            <tr>
                <td><s:property value='%{#weekly["start_date"]}'/> - <s:property value='%{#weekly["end_date"]}'/></td>
                ...
                <s:iterator value="columns" var="col">
                    ...
                    <s:elseif test="%{#col.group==1}">
                        <td class="text-right" service-group='<s:property value="%{#col.serviceId}" />' group="inc-gst">
                            <s:property value="%{#weekly[#col.fieldName]}"/></td>
                    </s:elseif>
                    <s:elseif test="%{#col.group==2}">
                        <td class="text-right" service-group='<s:property value="%{#col.serviceId}" />' group="exc-gst">
                            <s:property value="%{#weekly[#col.fieldName]}"/></td>
                    </s:elseif>
                </s:iterator>
        ...
        <s:if test="weeklyTotal!=null">
            <tr>
                ...
                <s:iterator value="columns" var="col">
                    ...
                    <s:elseif test="%{#col.group==1}">
                        <th class="text-right" service-group='<s:property value="%{#col.serviceId}" />' group="inc-gst">
                            <s:property value="%{weeklyTotal[#col.fieldName]}"/></th>
                    </s:elseif>
                    <s:elseif test="%{#col.group==2}">
                        <th class="text-right" service-group='<s:property value="%{#col.serviceId}" />' group="exc-gst">
                            <s:property value="%{weeklyTotal[#col.fieldName]}"/></th>
                    </s:elseif>
                </s:iterator>
                <th class="text-right"><s:property value='%{weeklyTotal["total_shipment_count"]}'/></th>
                <th class="text-right" group="inc-gst"><s:property value='%{weeklyTotal["total_rev_inc_gst"]}'/></th>
                <th class="text-right" group="exc-gst"><s:property value='%{weeklyTotal["total_rev_exc_gst"]}'/></th>
                <th class="text-right" group="inc-gst"><s:property value='%{weeklyTotal["total_margin_inc_gst"]}'/></th>
                <th class="text-right" group="exc-gst"><s:property value='%{weeklyTotal["total_margin_exc_gst"]}'/></th>
            </tr>
        </s:if>
    </s:else>
    </tbody>
