...
<table class="table table-bordered mg0" id="monthly_report_table">
    ...
    <s:else>
        <s:iterator value="monthlyReport" var="monthly">
            <tr>
                <td><s:property value='%{#monthly["start_date"]}'/> - <s:property value='%{#monthly["end_date"]}'/></td>
                ...
                <s:iterator value="columns" var="col">
                    ...
                    <s:elseif test="%{#col.group==1}">
                        <td class="text-right" service-group='<s:property value="%{#col.serviceId}" />' group="inc-gst">
                            <s:property value="%{#monthly[#col.fieldName]}"/></td>
                    </s:elseif>
                    <s:elseif test="%{#col.group==2}">
                ...
                <td class="text-right" group="inc-gst"><s:property value='%{#monthly["total_margin_inc_gst"]}'/></td>
                <td class="text-right" group="exc-gst"><s:property value='%{#monthly["total_margin_exc_gst"]}'/></td>
            </tr>
        </s:iterator>
        <s:if test="monthlyTotal!=null">
            <tr>
                ...
                <s:iterator value="columns" var="col">
                    ...
                    <s:elseif test="%{#col.group==1}">
                        <th class="text-right" service-group='<s:property value="%{#col.serviceId}" />' group="inc-gst">
                            <s:property value="%{monthlyTotal[#col.fieldName]}"/></th>
                    </s:elseif>
                    <s:elseif test="%{#col.group==2}">
                ...
                <th class="text-right" group="inc-gst"><s:property value='%{monthlyTotal["total_rev_inc_gst"]}'/></th>
                <th class="text-right" group="exc-gst"><s:property value='%{monthlyTotal["total_rev_exc_gst"]}'/></th>
