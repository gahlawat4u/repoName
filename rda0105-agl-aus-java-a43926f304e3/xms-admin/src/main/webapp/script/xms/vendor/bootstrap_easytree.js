        ...
        this.each(function () {
            ...
            $.each($(easyTree).find('ul > li'), function () {
                ...
                if ($(this).is('li:has(ul)')) {
                    ...
                    $(this).find(' > span > span').addClass(' fa-folder ');
            ...
            if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        var item = $('<li><span><span class="glyphicon fa-file-o "></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                        ...
                        if (selected.length <= 0) {
                            ...
                            if ($(selected).hasClass('parent_li')) {
                                ...
                                $(selected).addClass('parent_li').find(' > span > span').addClass(' fa-folder-open ').removeClass('fa-file-o ');
            ...
            if (options.deletable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .remove > button').attr('title', options.i18n.deleteTip).click(function () {
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content').html(options.i18n.deleteConfirmation).append('<a style="margin-left: 10px;" class="btn btn-default btn-danger confirm"></a>').find('.confirm').html(options.i18n.confirmButtonLabel);
                        $(easyTree).find('.alert .alert-content .confirm').on('click', function () {
                            $(selected).find(' ul ').remove();
                            if ($(selected).parent('ul').find(' > li').length <= 1) {
                                $(selected).parents('li').removeClass('parent_li').find(' > span > span').removeClass(' fa-folder-open ').addClass('fa-file-o ');
            ...
            $(easyTree).delegate('li.parent_li > span', 'click', function (e) {
                ...
                if (children.is(':visible')) {
                    children.hide('fast');
                    $(this).attr('title', options.i18n.expandTip).find(' > span.fa').addClass('fa-folder').removeClass(' fa-folder-open ');
                } else {
                    children.show('fast');
                    $(this).attr('title', options.i18n.collapseTip).find(' > span.fa').addClass(' fa-folder-open ').removeClass('fa-folder');
                }
                e.stopPropagation();
            });
