...
public class TntPickupService {
    public String execute(String xmlRequest, String xmlResponseCached) {
        // Write xml-request to file.
        String uniqueString = UUID.randomUUID().toString().replace("-", "");
        AppUtils.writeToFileByDate(xmlRequest, "/tnt_intl_pickup_request_" + uniqueString + ".xml");
        ...
        if (result != null && "COMPLETE".equalsIgnoreCase(result[0])) {
            ...
            try {
                ...
                if (resultCode == 200) {
                    ...
                    AppUtils.writeToFileByDate(xmlResponse, "/tnt_intl_pickup_response_" + uniqueString + ".xml");
                } else {
                    xmlResponse = null;
                }
                return xmlResponse;
    ...
    public String prepareXmlRequest(TntShipmentRequestVo shipmentRequestVo) throws JAXBException {
        ...
        String paymentInd = "S";
        String receiverCountryCode = shipmentRequestVo.getShipmentInfo().getReceiverAddress().getCountryDetail().getCountryCode();
        if ("AU".equalsIgnoreCase(receiverCountryCode) && (shipmentRequestVo.getShipmentInfo().getSenderAddress().getCountry() != shipmentRequestVo.getShipmentInfo().getReceiverAddress().getCountry())) {
            paymentInd = "R";
        }
        ...
        // 4. CollInstructions.
        collection.setCOLLINSTRUCTIONS(shipmentRequestVo.getScheduleCollection().getSpecialInstructions());
        ...
        // Set customer ref.
        details.setCUSTOMERREF(shipmentRequestVo.getShipmentInfo().getReference());
        ...
        if (shipmentRequestVo.getShipmentInfo().getPieces() != null && shipmentRequestVo.getShipmentInfo().getPieces().size() > 0) {
            if (shipmentRequestVo.getShipmentInfo().getWeightUnit().equals("lb") && shipmentRequestVo.getShipmentInfo().getDimensionUnit().equals("in")) {
                for (PieceVo piece : shipmentRequestVo.getShipmentInfo().getPieces()) {
        ...
        details.setDELIVERYINST(shipmentRequestVo.getScheduleCollection().getSpecialInstructions());
        ...
        consignment.setCONREF(shipmentRequestVo.getShipmentInfo().getReference());
        ...
        // 1. Activity - Book.
        Book book = new Book();
        book.setShowBookingRef("Y");
        book.getCONREF().add(String.valueOf(shipmentRequestVo.getShipmentInfo().getShipmentId()));
        book.getCONNUMBER().add(shipmentRequestVo.getShipmentInfo().getAirbillNumber());
        activity.setBOOK(book);
        // 2. Activity - Ship.
        Ship ship = new Ship();
        ship.getCONREF().add(String.valueOf(shipmentRequestVo.getShipmentInfo().getShipmentId()));
