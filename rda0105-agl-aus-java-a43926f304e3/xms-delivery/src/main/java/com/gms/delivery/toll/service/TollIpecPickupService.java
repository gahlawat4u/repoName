...
/**
 * Posted from TollPickupService
 * <p>
 * Author dattrinh Feb 15, 2016 5:21:06 PM
 */
public class TollIpecPickupService {
    private static final Log log = LogFactory.getLog(TollIpecPickupService.class);
    ...
    public String execute(String xmlRequest, String xmlResponseCached) {
        ...
        AppUtils.writeToFileByDate(xmlRequest, "/toll_ipec_pickup_request_" + uniqueString + ".xml");
        ...
        try {
            ...
            if (resultCode == 200) {
                ...
                AppUtils.writeToFileByDate(xmlResponse, "/toll_ipec_pickup_response_" + uniqueString + ".xml");
            } else {
                xmlResponse = null;
            }
            return xmlResponse;
    ...
    public String prepareXmlRequest(TollIpecPickupRequestVo pickupRequestVo) throws JAXBException {
        ...
        pickupRequest.setCarrierID("IPEC");
        ...
        // Get Open Time
        String openTime = pickupRequestVo.getScheduleCollection().getPickupTime();
        if (openTime.length() <= 5) {
            openTime = openTime + ":00";
        }
        openTime = pickupDate + "T" + openTime + timeZone;
        pickupDetail.setOpenTime(openTime);
        // Get Close Time
        String closeTime = pickupRequestVo.getScheduleCollection().getPickupTimeNoLater();
        if (closeTime.length() <= 5) {
            closeTime = closeTime + ":00";
        }
        closeTime = pickupDate + "T" + closeTime + timeZone;
